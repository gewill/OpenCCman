...
设计入口：[完整画稿](README.md)、[UI 与交互规范](UI-SPEC.md)。状态：已选第 2 稿，当前交付详细设计/计划及 issues，**没有启动应用实现**。设计 PR 合入 `develop` 后，每项实现从最新 `develop` 新建 `codex/<issue>-<topic>`，按下列依赖交付。
...
5. 日常 PR 合入 `develop`。创建 PR 后报告检查状态，不自动合并或触发 Xcode Cloud；确需发布包时另行按既有发布流程执行。
