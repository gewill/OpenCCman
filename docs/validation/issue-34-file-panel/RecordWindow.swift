...
@main struct RecordOwnedApp {
  @MainActor static func main() async throws {
    ...
    let bundle = "org.gewill.OpenCCman.WhatsNewQA20260924"
    ...
    guard let app = content.applications.first(where: { $0.bundleIdentifier == bundle }),
          let window = content.windows.filter({ $0.owningApplication?.processID == app.processID && $0.frame.width >= 300 }).max(by: { $0.frame.width * $0.frame.height < $1.frame.width * $1.frame.height }),
    ...
    if !status.isEnded { try await stream.stopCapture() }
