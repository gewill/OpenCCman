...
[机器可读结果](2026-09-16/results.json) 包含版本、SDK 声明的有效部署目标、四项命令/退出码、源文件哈希。原始诊断日志与结果在同目录。`expectationsMatched=true` 表示“旧模式失败、新模式成功”符合实验预期，**不是四项编译都成功**。没有本机 Xcode 26.3，因此未虚构同机旧工具链对照；现有 Xcode 26.3 应用 CI 的成功只作为独立的整体验证证据。
