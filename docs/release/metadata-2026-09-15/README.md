...
- [ ] 核实 RC 后台集成、身份关联、实际用途及追踪，将问卷与源代码及候选政策对齐。
...
| 购买记录用途 | 无 | App Functionality 与 Analytics 为 RC 官方指南要求核对的用途，尚需结合项目实际后台使用确认 |
| 与身份关联 | 无对应数据项 | app 未配置自定义身份；不能仅由这一点排除后台或历史集成的关联，待核实 |
| 用于追踪 | 无对应数据项 | app 源码未发现广告标识采集；后台集成尚未核实，不预填“否”并发布 |
