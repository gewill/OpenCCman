...
这证明了闭包的分配来源和闭包到宿主这一段强引用，**没有证明完整持有根因、闭包最终所有者或真实 OpenCCman 的同一问题**。不据此清空文稿、改写业务模型、移除系统通知或禁用无障碍。
...
1. 在实际 OpenCCman 的隔离诊断候选中验证相同宿主/block 路径；最小程序不能替代实际应用证据。
2. 对比同工具链、同观察协议下无第二窗 AX 查询的存活对象，继续确定 registrar/窗口框架对象的所有权及清理时机。分配栈记录与 heap capture 均需作为独立实验条件。
