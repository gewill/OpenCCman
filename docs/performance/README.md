...
[2026-09-23 七配置内存来源诊断](2026-09-23-engine-memory/README.md)在相同应用阶段比较旧／新引擎的 `vmmap`、`heap` 和分配栈，并归档两项缓存上限的独立试验。词典与前缀匹配构建是主要来源；缓存上限没有足够的稳定收益，本次没有改变正式缓存策略。该证据不替代 #68 的 TextKit 2 大文稿剖析。

[2026-09-23 原生 TextKit 1／2 大文稿对照](2026-09-23-native-textkit/README.md)将单个原生编辑器与应用桥接分开，覆盖短段、长段和无换行单段；10 MiB 无换行首显超时和 5 MiB TextKit 2 的高 RSS 均保留为限制证据，#68 尚未完成。

[2026-09-23 当前应用 TextKit 桥接与变宽归因](2026-09-23-textkit-app-bridge/README.md)在同机三轮应用对照中定位文末布局切换的停顿，并用切换前后堆快照及原生单编辑器复现缩小触发条件；仍需可保留阅读位置的现代锚点候选与完整交互验收。

[2026-09-23 TextKit 2 可见字符锚点诊断](2026-09-23-textkit-anchor/README.md)在私有应用快照中加入现代 API 锚点，同机三组各三轮验证文末布局的时间／内存改善，同时发现 TextKit 2 文中目标滚动仍未进入可视区；候选尚未进入正式 App。
