...
| 10 MiB 结果编辑器更新 | 631.7 ms | 651.4 ms | 升级引擎没有解决显示成本 |
...
10 MiB 在当前引擎下，温文件读取约 10.7 ms，原文编辑器更新约 855.7 ms。当前主要成本已经移向文字显示。两组所有转换阶段的输入和输出哈希一致；七组有效配置的固定答案、U+0000、CRLF、空行、Emoji、组合字符、重复结果和导出快照断言通过。
...
CPU 在该协议下减少约 70.3%，首屏显示没有改善，不能表述为所有交互快了 70%。所有输出一致。原始 [基线样本](idle-layout-on/)、[试验样本](idle-layout-off/)、[比较结果](idle-layout-comparison.json) 保留完整范围及来源。
...
| 基线 | 闲时排版关闭试验（未采用） |
|---|---|
| ![基线运行](media/baseline.png) | ![试验运行](media/trial.png) |
| [布局与转换交互视频](media/baseline.mp4) | [布局与转换交互视频](media/trial.mp4) |

录像单独进行，不计入正式性能样本，也没有完整记录后续文末重排长等待。媒体经 `gh` 上传；[文件校验和与录制来源](media-provenance.json) 记录原始/压缩文件。完整 Instruments 包未上传。
