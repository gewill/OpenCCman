...
[默认 s2t 的修复后 Services 对照](optimized-s2t/README.md)已完成：匹配 `-O` 调用工具、相同 Release 工具链与语料，40 次输出正确；10 MiB 往返中位数约 419 → 314 ms。首轮调用工具参数不一致的记录完整保留但不作为正式对照。该结果不替代真实目标 App、其他配置或签名运行验收。
