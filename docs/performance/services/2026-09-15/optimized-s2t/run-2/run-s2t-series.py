...
command=subprocess.check_output(['ps','-p','26104','-o','command='],text=True).strip()
...
out=root/'optimized-6de377c'/'run-2'/'s2t-series'; out.mkdir(exist_ok=False)
(out/'context.json').write_text(json.dumps({'mode':mode,'uiSelectedConfiguration':observed,'providerPID':26104,'sourceSHA':'6de377c3f8f1973ba06b04fc489b7125caa613ea','startUTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),'providerState':'already launched, one window open, configuration sheet dismissed','sequence':'one 1 KiB priming call then eight cases x five consecutive calls; no artificial waits','corpusManifestSHA256':hashlib.sha256((corpus/'manifest.json').read_bytes()).hexdigest()},indent=2)+'\n')
probe=str(root/'optimized-6de377c'/'run-2'/'services-wait-probe')
