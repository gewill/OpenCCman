...
初始实现为 `db77088`；`3b16fe9` 将菜单验证限制为唯一匹配项所属菜单，避免刷新窗口列表。最终候选的 [Release 构建与三轮运行](https://github.com/gewill/OpenCCman/actions/runs/34997253528)已发起，实际结果待补。合成单元测试仅验证协议失败分支，不是真实运行证据。
