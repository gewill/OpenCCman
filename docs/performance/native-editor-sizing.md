...
此前 `b0addf7` 的截图、视频和同机性能结果仍是该提交的历史证据。本次滚动修复的独立 CI、同机文稿协议及实际 UI 结果见下节；#108 继续 Draft，完整交互、最低系统和发布门槛不变。

## 滚动修复后的复测与实际界面

当前应用源码 `d8bcb5c7c08e86347f739aa9f8a4791caf965164` 的[完整 App Regression](https://github.com/gewill/OpenCCman/actions/runs/35025127160)通过，包含原有有窗口滚动检查、新增生产编辑器检查、Introspection、StoreKit、额度及 macOS 双架构构建。[独立构建/对照运行](https://github.com/gewill/OpenCCman/actions/runs/35025165546)也通过：Xcode 26.3、macOS 15.7.9、Release、相同依赖锁，同一 runner 先完成两份构建，再按 `cb73fc5` → `d8bcb5c` 顺序运行，无 CUA、录屏或栈采样。

| 固定阶段 | 旧版 a / b | 本次候选 a / b |
|---|---:|---:|
| 1 MiB 导入到驱动就绪 | 2,382 / 2,558 ms | 34 / 136 ms |
| 1 MiB 转换到导出验证 | 2,535 / 2,278 ms | 217 / 184 ms |
| 10 MiB 导入到驱动就绪 | 22,829 / 20,977 ms | 181 / 175 ms |
| 10 MiB 转换到导出验证 | 23,177 / 24,074 ms | 478 / 472 ms |
| 其中 10 MiB 原生转换 | 209 / 258 ms | 213 / 226 ms |
| 采样峰值 footprint | 441,815,040 bytes | 169,250,368 bytes |
| 内核峰值 RSS | 771,588,096 bytes | 309,673,984 bytes |

下载后重新使用 checker 验证原始 JSONL 与九份实际语料/导出文件：两版各五份导出全部逐字节正确，无 BOM，NUL、CRLF、组合文字保留；活动转换中关窗与保留窗口自身转换时关闭另一窗通过，最终 +5/+20 秒零业务模型、零额度预约，进程正常退出，内存 API 有效。[新对照的原始证据、独立复核结果和文件 ZIP](native-editor-sizing/2026-09-16-anchor-comparison/)均已归档。这仍是单次顺序对照、固定配置和语料，不等于冷启动、屏幕呈现、七配置或发行版验收。前两组运行属于不同 runner，不能用它们与本次候选耗时相减来衡量锚点修复的额外成本。

本机真实 UI 使用 `b0addf7` 原生编辑器候选与 `d8bcb5c` 做同条件回归：macOS 27.0、English/Light/默认字号，相同 ad-hoc 签名和产品 entitlement，1302×768px 实际窗口截图（不替代 pt 测量）。固定 1,048,550 字节文稿，先通过原生滚动条定位中段，再选择第 804 段，拖动分隔条 50%→66% 并切换上下排列。两版均保留该路径的可见段落和选区；失败复现是前节无窗口用例，不能将相同截图说成“界面缺陷消失”。重复英文行不提供每个字符的独立视觉标记，图片证明范围限于可见段落。

| 分隔条拖动后：修复前 | 分隔条拖动后：修复后 |
|---|---|
| ![修复前](https://github.com/user-attachments/assets/0f2aee19-2ac7-4b55-98f6-afea6fee51cc) | ![修复后](https://github.com/user-attachments/assets/ddabdf5c-b8e5-45ac-b888-40bdb064a66e) |

[修复前交互](https://github.com/user-attachments/assets/bd00b6d5-1a83-4af2-ae30-3c2a9373183d)为 43.82 秒、自有应用过滤的显示器画布；[修复后完整补录](https://github.com/user-attachments/assets/77420366-d75f-4c14-a0a0-a08c7653bb2a)为 17.50 秒、自有窗口画布，两者都未剪辑。一次候选录制约 4 秒即报 ScreenCaptureKit -3822，中断片段不作为完整验收；新窗口录制工具显式初始化 AppKit，补录完整结束、退出 0。录制日志、工具源码、完整资产 URL/哈希与[实际 UI 记录](native-editor-sizing/2026-09-16-anchor-ui/)保存了这些边界。

本次还通过分隔条辅助递增/递减 50→55→50、深色简体/繁体原生编辑器标签与显示，并实际转换后经系统 Export 面板保存，导出 1,048,550 字节与固定原文完全相同，保留 3,350 个 CRLF、Emoji 和组合文字、无 BOM。这份语料在默认转换下字符不变，不替代转换词组语料。诊断额度成功扣 1 次；两款验收应用已退出并停用，标准/沙盒偏好恢复，VoiceOver 未开启且保持关闭，保护中的其他应用/设备未操作。

新观察：从设置页返回工作区会重建阅读位置、回到文稿开头，未标为滚动保留通过；全文未丢失，之后导出的完整字节已验证。原生工厂测试不能证明跨路由页面保留编辑状态，后续需在基线和候选上独立定位。Cmd-F 在此次基线没有打开查找栏，也未标记快捷键查找已验收。真实输入法、完整 VoiceOver/键盘、任务中布局切换、窄窗口外层裁切、移动端及最低系统仍保留，#18/#20/#65/#93 不因本次结果关闭。

## 纯键盘补验与剩余人工边界

2026-09-16 在相同 `d8bcb5c` 诊断包完成 Tab/Space 布局操作、Cmd-T 转换、原文/只读结果切换、系统文件导出、非法 UTF-8 导入错误确认，以及预设选择/Escape 退出。两次成功转换扣 2 次，实际导出准确为 5 字节 `nihao`；失败导入没有替换原稿或旧结果。当前文档 HEAD `eb2dd51` 的 [App Regression](https://github.com/gewill/OpenCCman/actions/runs/35027501496) 通过。

[范围、焦点摘录、媒体来源和恢复记录](native-editor-sizing/2026-09-16-keyboard-ui/)已归档。Tab 经过 Paste → Copy → Export → Import → Source，视觉分组顺序仍待查；工具输入只得到 ASCII，没有证明中文组合文字。VoiceOver 虽实际开启，但未取得可靠导航/朗读证据，已立即关闭。本轮录屏再次 -3822 中断，未用残片充当完整交互视频。全套 #20 仍开放，最低系统保留 #16 发布门槛。系统键盘导航、输入法及诊断偏好已恢复，诊断包退出停用。
