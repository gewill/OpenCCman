...
def validate(run, mode, size):
    ...
    for row in run['rows']:
        ...
        if row['name'].startswith('scroll') and (not row['target_visible'] or row['scroll_attempts'] < 1):
...
def main():
    ...
    if output == ROOT or ROOT in output.parents or output.exists() or args.samples < 1 or args.timeout < 1 or any(s < 1 or s > 10 for s in args.sizes):
        ...
        for sample in range(1, args.samples + 1):
            for mode in args.modes:
                ...
                try:
                    ...
                    validate(run, mode, mib * 1024 * 1024)
